...
passcount+=(toSim('樹幹')=='树幹')?1:0            //safe
passcount+=(toSim('樹幹',true)=='树干')?1:0       //unsafe
passcount+=(toSim('心願',true)=='心愿')?1:0       //safe
passcount+=(toSim('餅乾')=='饼乾')?1:0            //safe
passcount+=(toSim('餅乾',true)=='饼干')?1:0       //unsafe
 
passcount+=(fromSim('饼干')  =='餅干')?1:0          //safe
passcount+=(fromSim('饼干',1)=='餅乾')?1:0        //pick the best match
passcount+=(fromSim('饼干',2)=='餅[乾幹干]')?1:0  //expand other possibility


passcount+= (fromSim(toSim(text))==text)?1:0;
passcount+= (fromSim(toSim(text2))==text2)?1:0;
passcount+= (fromSim(toSim(text3))==text3)?1:0;
